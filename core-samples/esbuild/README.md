...
- The sample includes a workaround for a known esbuild bug that duplicates CSS: https://github.com/evanw/esbuild/issues/3529. The bug causes the Calcite Components CSS to be duplicated which increases the size of on-disk build output by up to 10MB, as well as adds approximately 277 additional CSS files. The workaround uses the `postbuild` script included in the package.json file to delete the duplicate files.
