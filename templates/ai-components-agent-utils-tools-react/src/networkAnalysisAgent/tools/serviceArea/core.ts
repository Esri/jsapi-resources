 ...
 */
export const findServiceAreas = async (options: FindServiceAreasOptions, mapElement: ArcgisMap): Promise<string> => {
  const { facilities, driveTimeCutoffs, travelModeName, travelDirection } = options;
  ...
  const travelMode = networkDescription.supportedTravelModes?.find((travelMode) => travelMode.name === travelModeName);

  const serviceAreasLayer = mapElement
